...
class UpdateCardFrame(QFrame):
    ...
    def save_changes(self):
        ...
        try:
            old_filename = self.item_data['picture']
            new_filename = None
            ...
            if self.new_picture_path:
                ...
                # --- 2. Удаляем старое фото, ТОЛЬКО если оно существует и не дефолтное ---
                if old_filename and old_filename != "picture.png":
                    old_full_path = os.path.join(self.ICONS_DIR, old_filename)
                    if os.path.exists(old_full_path):
                        try:
                            os.remove(old_full_path)
                            print(f"Удалено старое фото: {old_filename}")
                        except OSError as e:
                            print(f"Не удалось удалить старое фото {old_filename}: {e}")
                            # Не прерываем выполнение — продолжаем сохранять новое

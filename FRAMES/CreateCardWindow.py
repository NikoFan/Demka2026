...
class CreateCardFrame(QFrame):
    ...
    def setup_ui(self):
        ...
        """
        Проблема создания в том, что в Задании указаны
            не все поля, для ввода
        С одной стороны - это косяк для НАС,
            т.к. мы должны соответствовать заданию
        С другой стороны - Мы не создадим новый товар
            не указав ему article
        Поэтотму приходится делать костыли...
        """
        ...
        btn = QPushButton("Сохранить новый товар")
    ...
    def save_changes(self):
        ...
        try:
            ...
            if self.new_picture_path:
                ...
                new_filename = f"{user_input_list[0]}{ext}"
    ...
    def go_back_to_log_in_window(self):
        ...
        if send_I_message("Вы точно прекратить процесс добавления товара?") < 20_000:
            self.controller.switch_window(HomePageWindow.HomeFrame)
